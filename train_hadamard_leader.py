"""
Hadamard FFN variant of the Int6+NorMuon+FA3 leader.

Only change: MLP up-projection replaced with fixed Hadamard expansion + trained diagonal.
Down-projection stays dense. Everything else identical to the leader.

Set HADAMARD_FFN=1 and MLP_HIDDEN=3072 (6x) to test iso-artifact Hadamard vs leader's 3x dense.
"""
...
# Flash Attention 3 (Modal) or fallback to PyTorch SDPA
try:
...
class Hyperparameters:
    ...
    # Hadamard FFN: replace MLP up-projection with fixed Hadamard + trained diagonal.
    hadamard_ffn = bool(int(os.environ.get("HADAMARD_FFN", "0")))
    hadamard_seed_base = int(os.environ.get("HADAMARD_SEED_BASE", 12345))
...
def normuon_update(
    grad: Tensor, momentum: Tensor, second_momentum: Tensor,
    beta: float = 0.95, beta2: float = 0.95, ns_steps: int = 5, nesterov: bool = True,
) -> Tensor:
...
class NorMuon(torch.optim.Optimizer):
    def __init__(self, params, lr=0.02, weight_decay=0.0, momentum=0.95, beta2=0.95, backend_steps=5):
        defaults = dict(lr=lr, weight_decay=weight_decay, momentum=momentum, beta2=beta2, backend_steps=backend_steps)
            ...
            for base_i in range(0, len(params_pad), world_size):
                if base_i + rank < len(params):
                    ...
                    update = normuon_update(
                        ...
                        beta=group["momentum"], beta2=group["beta2"], ns_steps=group["backend_steps"],
                    )
...
class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: int, mlp_hidden: int = 0,
                 hadamard_ffn: bool = False, hadamard_seed: int = 0):
        super().__init__()
        hidden = mlp_hidden if mlp_hidden > 0 else mlp_mult * dim
        self.hadamard_ffn = hadamard_ffn
        if hadamard_ffn:
            if dim & (dim - 1) != 0:
                raise ValueError(f"Hadamard FFN requires dim to be power of 2, got {dim}")
            self.width_mult = hidden // dim
            if hidden % dim != 0:
                raise ValueError(f"MLP hidden ({hidden}) must be divisible by dim ({dim}) for Hadamard FFN")
            self.dim = dim
            # Fixed random signs per parallel copy — not saved in state_dict.
            gen = torch.Generator().manual_seed(hadamard_seed)
            signs = (torch.randint(0, 2, (self.width_mult, dim), generator=gen) * 2 - 1).float()
            self.register_buffer("signs", signs, persistent=False)
            # Trained diagonal after expansion.
            self.d_scale = nn.Parameter(torch.ones(hidden))
        else:
    ...
    def forward(self, x: Tensor) -> Tensor:
        if self.hadamard_ffn:
            batch_shape = x.shape[:-1]
            h = x.unsqueeze(-2) * self.signs.to(x.dtype)
            h = h.reshape(-1, self.dim)
            h = hadamard_transform(h)
            h = h.reshape(*batch_shape, self.width_mult * self.dim)
            h = h * self.d_scale.to(x.dtype)
            h = torch.relu(h)
...
class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
                 rope_base: float, qk_gain_init: float, mlp_hidden: int = 0,
                 hadamard_ffn: bool = False, hadamard_seed: int = 0):
        super().__init__()
        self.attn_norm = RMSNorm()
        self.mlp_norm = RMSNorm()
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init)
        self.mlp = MLP(dim, mlp_mult, mlp_hidden, hadamard_ffn=hadamard_ffn, hadamard_seed=hadamard_seed)
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, model_dim: int, num_heads: int,
                 ...
                 qk_gain_init: float, hadamard_ffn: bool = False, hadamard_seed_base: int = 12345):
        super().__init__()
        ...
        self.blocks = nn.ModuleList([
            Block(model_dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init,
                  mlp_hidden=mlp_hidden, hadamard_ffn=hadamard_ffn, hadamard_seed=hadamard_seed_base + i)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        hadamard_ffn=args.hadamard_ffn, hadamard_seed_base=args.hadamard_seed_base,
    ).to(device).bfloat16()
    ...
    log0(f"model_params:{n_params} hadamard_ffn:{args.hadamard_ffn}")
    ...
    log0(f"final_int8_zlib_roundtrip val_loss:{q_val_loss:.4f} val_bpb:{q_val_bpb:.4f} "
         f"eval_time:{1000.0 * (time.perf_counter() - t_qeval):.0f}ms eval_seq_len:{effective_eval_seq_len}")
    ...
    if args.eval_stride > 0:
        ...
        torch.cuda.synchronize()
        log0(f"final_sliding_window val_loss:{s_val_loss:.4f} val_bpb:{s_val_bpb:.4f} "
